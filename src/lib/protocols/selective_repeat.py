...
class WindowItem:
    ...
    data: bytes
    acked: bool = False
    retries_left: int = 10  # una menos que max_retries
...
class SelectiveRepeatProtocol:
    ...
    def _actualizar_window(self) -> None:
        if self.window.length() == 0:
            self.logger.debug("No hay paquetes en la ventana para actualizar.")
            return
    ...
    def ack_received(self, seq_number: int) -> bool:
        self.logger.debug("Llego ACK de seq_num: " + str(seq_number))
        try:
